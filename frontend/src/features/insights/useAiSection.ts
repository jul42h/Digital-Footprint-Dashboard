...
function matchesIntent(intent: SectionIntent, data: CveAnalysisResponse): boolean {
  ...
  // The Risk Score card pairs the prose rationale with the pipeline-computed
 ...
 * Highest-Risk Assets don't each hand-roll it.
 */
export function useAiSection(intent: SectionIntent, findings: AnalysisFinding[]) {
  ...
  const run = useCallback(
    async (bypassCache: boolean) => {
      if (!findings.length || inFlight.current) return;
      inFlight.current = true;
      setLoading(true);
      setError(null);
      try {
        const payload = await analyzeCves([], { intent, findings, bypassCache });
        accept(payload);
      } catch (err) {
        setData(null);
  ...
  useEffect(() => {
    ...
    const cached = peekCachedAnalysis([], intent);
    if (cached) {
      accept(cached);
      return;
    }
